  ...
  if (gate) {
    ...
    try {
      ...
      if (!poseOK && inProx && Number.isFinite(enter) && (frame - enter) <= 2) {
        ...
        if (window.viason_SHOT_DEBUG) console.log('[shot_arc] RELEASE LATCHED (prox-fallback)', { frame });
        _state.relLatched = true;
        return true;
      }
    } catch {}
    ...
    try {
      ...
      if (t.length >= 3 && H) {
        ...
        if (!poseOK && up && laneX && nearRimBand) {
          ...
          if (window.viason_SHOT_DEBUG) console.log('[shot_arc] RELEASE LATCHED (slope-fallback)', { frame });
          _state.relLatched = true;
          return true;
        }
      }
