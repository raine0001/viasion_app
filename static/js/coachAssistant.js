...
window.__coachMuted = JSON.parse(localStorage.getItem('visaion_muted') || 'false');
...
window.visaion_ONLY_REALTIME = true;
...
window.__onShotSummaryInternal = function (s) {
    ...
    try { (window.visaionSpeak || window.coachSpeak)?.(line); } catch { }
...
window.addEventListener('hud:start-session', () => {
    try { window.visaion_ONLY_REALTIME = false; } catch { }
});
...
window.addEventListener('hud:mute-toggle', (e) => {
    ...
    // 🔁 Keep visaionPrefs in sync so window.coachSpeak() won't skip
    try {
        const get = window.visaionGetPrefs?.() || {};
        const next = { ...get, audioOn: !muted };
        window.visaionSetPrefs?.(next);
    } catch { }
});
...
try {
    const rawPref = localStorage.getItem('visaion_tts');
    const parsed = rawPref ? JSON.parse(rawPref) : {};
    const voice = parsed.voice || (window.visaion && window.visaion.voice) || 'alloy';
    if (!parsed.provider || parsed.provider !== 'server') {
        localStorage.setItem('visaion_tts', JSON.stringify({ provider: 'server', voice }));
    }
} catch {
    try {
        const fallbackVoice = (window.visaion && window.visaion.voice) || 'alloy';
        localStorage.setItem('visaion_tts', JSON.stringify({ provider: 'server', voice: fallbackVoice }));
    } catch { }
}
...
try {
    if (!window.__poseOverlayWired) {
        ...
        window.addEventListener('shot:release', () => {
            try { if (window.visaion_RELEASE_TRACE === true) console.log('[coach:evt] shot:release'); } catch { }
            setTimeout(async () => {
                try {
...
(function () {
    ...
    window.composePoseFeedback = function composePoseFeedbackV3(snap) {
        try {
            const g = window.visaion_MEM?.get?.()?.golden || null;
...
window.addEventListener('visaion:session-review', (event) => {
    try {
...
function formatCoachLine(s) {
    ...
    try {
        ...
        const golden = window.visaion_MEM?.get?.()?.golden || null;
...
window.addEventListener('shot:summary', (e) => {
    ...
    // Build final line - visaion assessment
    ...
    // Speak once, here
    try { if (!window.__coachMuted) (window.visaionSpeak || window.coachSpeak)?.(personalized); } catch { }
...
(function () {
    ...
    window.visaion = visaion;
    if (typeof visaion.poseOnly === 'undefined') visaion.poseOnly = true;
    if (visaion.poseOnly) visaion.llmMode = 'off';
    try { if (typeof window.visaion_ONLY_REALTIME === 'undefined') window.visaion_ONLY_REALTIME = true; } catch { }
    console.log('[visaion] coachAssistant loaded');

    // Prevent double-initialization if the script is included twice
    if (window.__visaion_INIT__) return;
    window.__visaion_INIT__ = true;
    ...
    function assessPoseAndSpeak(via) {
        try {
            ...
            if (!armed || !confirmed) {
                if (window.visaion_RELEASE_TRACE === true) { try { console.log('[coach:tip:skip]', { via, reason: !armed ? 'not-armed' : 'no-hoop' }); } catch { } }
                return;
            }
                                ...
                                try {
                                    ...
                                    if (!s2) {
                                        try {
                                            ...
                                            if (hasAny) {
                                                if (window.visaion_RELEASE_TRACE === true) console.log('[coach:tip:minimal]', { via, snap: mSnap, gate: t });
                                                ...
                                                try { if (window.visaion_RELEASE_TRACE === true) console.warn('[coach:tip:no-snapshot]'); } catch { }
                                            }
                                        } catch { }
                                        return;
                                    }

                                    const golden2 = window.visaion_MEM?.get?.()?.golden;
                                    ...
                                    if (window.visaion_RELEASE_TRACE === true) console.log('[coach:tip:resample]', { via, snap: s2, issues: issues2, lastGate: window.__LAST_GATE?.detail?.tests || null });
    ...
    async function speakWithAIOrRules(snap, via) {
        ...
        function postDisconnected() {
            try {
                const msg = 'visaion is not connected. Please restart the session and check your internet connection.';
                window.showPromptMessage?.(msg, 2000);
                console.warn('[coach:ai:error] not connected');
            } catch { }
        }

        const llmMode = (window.visaion && window.visaion.llmMode) || 'off';
        ...
        if (llmMode === 'off') {
            try {
                ...
                if (local) {
                    ...
                    try { if (window.visaion_RELEASE_TRACE === true) console.log('[coach:speak:off]', { via, out }); } catch { }
        ...
        try {
            ...
            const body = {
                ...
                model: (window.visaion && window.visaion.model) || 'gpt-4o-mini',
                lang: 'en-US',
                shot: snap,
                profile: (localStorage.getItem('visaionProfile') || ''),
                sid: (window.__SESSION_ID || null),
                shotId: inferShotIdx0(),
            };
            if (window.visaion_RELEASE_TRACE === true) console.log('[coach:ai:req]', { via, ms, body });
            ...
            if (window.visaion_RELEASE_TRACE === true) console.log('[coach:ai:res]', { via, textLen: text.length, text });
            if (text) {
                try {
            ...
            try {
                ...
                if (local) {
                    const out = withShotPrefix(local);
                    window.__lastCoachText = out;
                    try { if (window.visaion_RELEASE_TRACE === true) console.log('[coach:speak:fallback-local]', { via, out }); } catch { }
                    try { setCoachNotesContent(out); } catch { }
                    return;
                }
            } catch { }
            ...
            if (window.visaion_RELEASE_TRACE === true) console.warn('[coach:ai:error]', e?.message || e);
            ...
            try {
                ...
                if (local) {
                    const out = withShotPrefix(local);
                    window.__lastCoachText = out;
                    try { if (window.visaion_RELEASE_TRACE === true) console.log('[coach:speak:error-local]', { via, out }); } catch { }
                    try { setCoachNotesContent(out); } catch { }
                    return;
                }
            } catch { }
    ...
    try {
        if (!window.__coachReleaseWired) {
            ...
            window.addEventListener('shot:release', () => { try { if (window.visaion_RELEASE_TRACE === true) console.log('[coach:evt] shot:release'); } catch { } try { window.__COACH_HAS_RELEASE = true; window.__COACH_TIP_LAST_AT = 0; } catch { } try { if (window.visaion_RELEASE_TRACE === true) console.log('[coach:evt] shot:release'); } catch { } assessPoseAndSpeak('shot:release'); });
            ...
            window.addEventListener('hud:shot-taken', () => {
                try {
                    ...
                    if (window.visaion_RELEASE_TRACE === true) { try { console.log('[coach:evt] hud:shot-taken', { now, last }); } catch { } }
            ...
            window.addEventListener('hud:score-trip', async () => {
                try {
                    if (window.__shotTrackingArmed !== true || window.__hoopConfirmed !== true) return;
                    const now = performance.now(); if (window.visaion_RELEASE_TRACE === true) { try { console.log('[coach:evt] hud:score-trip'); } catch { } }
                    const last = Number(window.__COACH_LAST_REL_SPEAK || 0);
            ...
            window.addEventListener('shot:release', () => { try { if (window.visaion_RELEASE_TRACE === true) console.log('[coach:evt] shot:release'); } catch { } try { __lastSpokenKey = null; window.__COACH_TIP_LAST_AT = 0; } catch { } });
        ...
        try {
            ...
            if (slug === 'golf') {
                ...
                try {
                    ...
                    if (shots.length) {
                        const g = (window.visaion_MEM?.get?.()?.golden) || { stanceWidthFeet: 120, kneeFlex: 28, toeToHoopDeg: 18, feetAngleDiff: 8, feetStagger: 6, shoulderToWristAngle: 55, releaseAboveShoulder: true };
            ...
            try {
                window.dispatchEvent(new CustomEvent('visaion:session-review', {
            ...
            try {
                const speakJob = (window.visaionSpeak || window.coachSpeak)?.(out);
    ...
    // ---- Pref bridges (new) ----
    // Read new UI prefs if present; fall back to older visaionPrefs values.
    function isAudioOn() {
        if (typeof window.PREF_AUDIO_ENABLED !== 'undefined') return !!window.PREF_AUDIO_ENABLED;
        const p = visaionGetPrefs();                 // legacy store
    ...
    function isMicAllowed() {
        ...
        const p = visaionGetPrefs();
        return (p.allowMic !== false);             // default true
    }


    // ---------- Prefs + Presets ----------
    const LS_KEY = 'visaionPrefs';

    const getAC = () => (window.__visaionAC ||= new (window.AudioContext || window.webkitAudioContext)());


    function visaionGetPrefs() {
        try { return JSON.parse(localStorage.getItem(LS_KEY)) || {}; }
        catch { return {}; }
    }

    function visaionSetPrefs(p) {
        const v = p || {};
        localStorage.setItem(LS_KEY, JSON.stringify(v));
        window.__visaionPrefs = v;
        return v;
    }

    //  visaion Memory   ------------------------------------- //
    const MEM_KEY = 'visaionMemoryV1';
    ...
    window.handleVoiceTranscript = async (text) => {
        ...
        if (visaionSpeak) coachSpeak(`You said: ${text}`);
        // window.webkit?.messageHandlers?.visaion?.postMessage({action: 'startVoice'})
    };

    // Export on window
    window.visaionGetPrefs = visaionGetPrefs;
    window.visaionSetPrefs = visaionSetPrefs;

    function getPrefs() { try { return JSON.parse(localStorage.getItem(LS_KEY)) || {}; } catch { return {}; } }
    function setPrefs(p) { localStorage.setItem(LS_KEY, JSON.stringify(p)); window.__visaionPrefs = p; return p; }
    ...
    window.visaionLoadPresets = loadPresets; window.visaionSavePreset = savePreset; window.visaionDeletePreset = deletePreset;
    ...
    window.visaionListWebVoices = (lang = '') => {
    ...
    async function ttsFetchBlob(text, voice) {
        const res = await fetch(visaion.ttsEndpoint, {
    ...
    function composeLLMPrompt(shot, golden, draftLine, made, personality) {
        ...
        return `
  You are visaion, a ${personality} shooting coach.
...
Draft to refine (optional): '${draftLine}'` : ''}
  ...
  `.trim();
    ...
    // analyze shot pose
    window.visaionOnShot = async function (shot) {
        try {
            ...
            const mem = window.visaion_MEM.get();
            const golden = mem.golden;
            const made = !!shot.made;
            const poseOnly = visaion.poseOnly === true;
            ...
            const mode = (window.visaion?.llmMode || 'polish').toLowerCase();
            let text = localText;
            const inferShotIdx0 = () => {
                try { if (Number.isFinite(Number(shot?.coachIdx))) return Number(shot.coachIdx); } catch { }
                try { if (Number.isFinite(Number(shot?.idx))) return Number(shot.idx); } catch { }
            ...
            if (!poseOnly && mode !== 'off' && window.visaion?.chatEndpoint) {
                try {
                    const prompt = composeLLMPrompt(
                        shot, golden,
                        mode === 'polish' ? localText : '', // primary: no draft; polish: send draft
                        made,
                        window.visaion?.personality || 'positive, concise'
                    );
                    const r = await fetch(window.visaion.chatEndpoint, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ prompt, model: window.visaion.model, temperature: 0.8 })
                    });
                    const j = await r.json();
            ...
            } catch (e) { console.warn('[visaion] coach text UI update failed:', e); }
            ...
            //  if (!window.visaion_ONLY_REALTIME) {
            //      queueMicrotask(() => visaionSpeak?.(text));
            // }

        } catch (e) { console.warn('[visaionOnShot]', e); }
    };
    ...
    window.updateCoachNotes = function updateCoachNotes(shot) {
        ...
        let html = `
      <strong> visaion Feedback</strong><br>
    ...
    // -----------------------------------------------
    // Hands-Free visaion (standalone, no global collisions)
    // Exposes: window.visaionHandsFree.start(), .stop(), .toggle(), .isActive()
    // -----------------------------------------------
    (() => {
        if (window.__visaionHFInit) return;          // prevent duplicate init
        window.__visaionHFInit = true;

        const ua = (navigator.userAgent || '').toLowerCase();
        if (/android/.test(ua) && window.visaion_ENABLE_ANDROID_SR !== true) {
            console.warn('[visaion HF] SpeechRecognition disabled on Android; set visaion_ENABLE_ANDROID_SR=true to override');
            window.visaionHandsFree = { start() { }, stop() { }, toggle() { }, isActive: () => false };
            return;
        }
        ...
        if (!SR) {
            console.warn('[visaion HF] Web Speech API not available');
            window.visaionHandsFree = { start() { }, stop() { }, toggle() { }, isActive: () => false };
            return;
        }
        ...
        async function start() {
            ...
            const ua = (navigator.userAgent || '').toLowerCase();
            if (/android/.test(ua) && window.visaion_ENABLE_ANDROID_SR !== true) return;
            // permission prime (helps UX)
            if (!window.__visaionMicPrimed) {
                try {
                    await navigator.mediaDevices.getUserMedia({ audio: true });
                    window.__visaionMicPrimed = true;
                } catch (err) {
                    try { console.warn('[visaion HandsFree] mic prime rejected', err); } catch { }
                    return;
                }
            }
            ...
            hfRec.onresult = (e) => {
                ...
                setCoachNotesContent(
                    `<strong>🎙 You:</strong> ${transcript}<br><strong>🤖 visaion:</strong> ${reply}`,
                    { html: true }
                );
            };
            ...
            visaionSpeak?.("Listening. Ask about feet, release, power, arc, or pose adjustments.");
        }
    ...
    // Adds poseSnapshot if missing, then calls visaion_MEM.addShot() and updateCoachNotes()
    window.addEventListener('shot:summary', (e) => {
        // If we've already handled THIS object, bail (covers re-dispatch)
        if (e.detail && e.detail.__visaionHandled) return;
        ...
        if (shot) shot.__visaionHandled = true;
        rememberKey(key);

        console.log('[visaion] shot:summary handled key=', key, shot);
        ...
        window.visaion_MEM.addShot(cloned);
        window.updateCoachNotes?.(cloned);
        // Attach per-shot coach line for the table; respect voice preference inside visaionOnShot
        window.visaionOnShot?.(cloned);
    });



    // ───────────────────────────────────────────────
    // visaion Voice Q&A (single, hardened instance)
    // ───────────────────────────────────────────────
    (function () {
        const ua = (navigator.userAgent || '').toLowerCase();
        if (/android/.test(ua) && window.visaion_ENABLE_ANDROID_SR !== true) {
            console.warn('[visaion Voice] SpeechRecognition disabled on Android; set visaion_ENABLE_ANDROID_SR=true to override');
            try { window.__startCoachVoiceRecognition = () => false; } catch { }
            return;
        }
        const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
        if (!SR) { console.warn('[visaion Voice] SpeechRecognition not supported'); return; }

        // Wake words (loose match)
        const visaion = (window.visaion ||= {});
        visaion.WAKE_WORDS ||= ['hey visaion', 'hey coach', 'my coach', 'coach', 'douch'];
        ...
        const prefs = (window.visaionGetPrefs?.() || {});
        ...
        const hasWake = (t) => {
            const n = norm(t);
            return visaion.WAKE_WORDS.some(w => n.includes(norm(w)));
        };

        function lastShot() { return window.visaion_MEM?.lastShot?.() || null; }
        ...
        function showDot(on) {
            ...
            if (!root) return;
            let dot = document.getElementById('visaionVoiceDot');
            if (!dot) {
                dot = document.createElement('div');
                dot.id = 'visaionVoiceDot';
        ...
        let allowIOSWake = !IS_IOS || !!window.__visaionMicPrimed || isMicAllowed();
        let pendingIOSWake = false;

        async function start(force = false) {
            const micAllowed = isMicAllowed();
            if (!micAllowed && !force) {
                console.warn('[visaion HF] mic disabled by preferences');
                return;
            }
            if (force && !micAllowed) {
                try { visaionSetPrefs({ ...visaionGetPrefs(), allowMic: true }); } catch { }
            }
            ...
            if ((!window.__visaionMicPrimed) && navigator.mediaDevices?.getUserMedia) {
                const ua = (navigator.userAgent || '').toLowerCase();
                const androidBlocked = /android/.test(ua) && window.visaion_ENABLE_ANDROID_SR !== true;
                if (!androidBlocked) {
                    try {
                        await navigator.mediaDevices.getUserMedia({ audio: true });
                        window.__visaionMicPrimed = true;
                        ...
                        try { console.warn('[visaion Voice] mic prime rejected', err); } catch { }
                        if (IS_IOS && !force) {
                            allowIOSWake = false;
                            pendingIOSWake = true;
                        }
        ...
        recog.onresult = async (ev) => {
            ...
            if (captureMode) {
                ...
                const wakeRe = new RegExp(visaion.WAKE_WORDS.map(w => norm(w)).join('|'), 'g');
                ...
                if (!/(feet|stance|release|wrist|elbow|power|knee|arc|entry|angle|make|accuracy)/.test(q) && visaion.chatEndpoint) {
                    try {
                        const ctx = { lastShot: lastShot(), recent: window.visaion_MEM?.recent?.(5) };
                        const r = await fetch(visaion.chatEndpoint, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                                prompt: `You are visaion. User asked: "${finalText}". Use this context JSON:\n${JSON.stringify(ctx)}\nGive a specific, actionable answer in 1-2 short sentences.`,
                                model: visaion.model
                            })
                        });
                        const j = await r.json();
        ...
        const queueAutoStart = () => {
            ...
            if (IS_IOS && !window.__visaionMicPrimed) {
                pendingIOSWake = true;
                return;
            }
            start();
        ...
        if (IS_IOS) {
            ...
            window.addEventListener('coach:voice-rec-start', (event) => {
                const ua = (navigator.userAgent || '').toLowerCase();
                if (/android/.test(ua) && window.visaion_ENABLE_ANDROID_SR !== true) {
                    console.warn('[visaion Voice] Android voice wake suppressed');
                    return;
                }
