...
async function stepOnce(videoEl, canvasEl, frameIdx, buf, bctx) {
            ...
            if (cand && hoopLocked) {
                ...
                if (last) {
                    ...
                    if (dist <= maxStep) { updateBall?.({ x: cand.x, y: cand.y }, frameIdx); updatedThisTick = true; }
                        ...
                        if (window.viasion_SHOT_DEBUG) console.log('[fbf] clamp ghost ball', { dist, maxStep, to: { x: cx, y: cy } });
    ...
    try {
        if (hoopLocked && (updatedThisTick || hasTrail)) {
            ...
            if (window.viasion_SHOT_DEBUG) {
                if (window.viasion_VERBOSE === true) console.log('[score:fbf]', frameIdx, { rel: window.ballState?.releaseFrame, enter: window.ballState?.proxEnterFrame, exit: window.ballState?.proxExitFrame, state: window.ballState?.state, shots: (window.shotLog?.length || 0) });
            }
        ...
        tickBusy = true; try {
            ...
            if (pick) {
                ...
                if (last) {
                    ...
                    } else if (window.viasion_SHOT_DEBUG) {
            ...
            try {
                ...
                if (window.viasion_SHOT_DEBUG) { const poseReady = !!(window.playerState?.keypoints?.length >= 33); const hasBall = !!(ballPt && Number.isFinite(ballPt.x)); console.log('[rvfc:tick] arcTick', { frame: fidx, poseReady, hasBall }); }
                if (hoopLocked && ballPt) {
            ...
            try {
                if (typeof window.releaseGate === 'function') {
                    ...
                    if (window.HUD_LOCAL_PULSE === true && window.__shotTrackingArmed === true && sc >= th - 1e-6 && frameIdx !== lastF) {
                        ...
                        if (window.viasion_RELEASE_TRACE === true) console.log('[score:pulse:an]', { frame: frameIdx, score: sc, th });
                    }
                }
            } catch { }
            try { updateDebugOverlay?.(poses, objects, frameIdx); } catch { };
