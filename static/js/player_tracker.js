...
export function updatePlayerTracker(landmarks, __frameIdx) {
    ...
    if (Number.isFinite(playerState.lastFrame) && frameNum < playerState.lastFrame) {
        ...
        try { if (window.viasion_VERBOSE === true && window.POSE_DEBUG === true) console.log('[pose:clamp-regress]', { was: __frameIdx, clampTo: frameNum, delta: df }); } catch (e) { }
    }
    ...
    // Pose debug logging
    if (window.viasion_VERBOSE === true && window.POSE_DEBUG === true) {
...
export function markRelease(frameIndex, opts = {}) {
    ...
    try {
        ...
        if (requirePose) {
            ...
            if (!kps) {
                if (window.viasion_SHOT_DEBUG) console.warn('[player_tracker] markRelease skipped - requirePose but no keypoints', { via, frame: lastFrame });
                return false;
            }
        }
    } catch (e) { }
