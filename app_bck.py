...
def upsert_shot(sess, sid: str, shot_idx: int, fields: dict):
    ...
    ShotsTbl = (
        db["ShotRow"].__table__
        if db.get("ShotRow")
        else Table("shots", MetaData(), autoload_with=db["engine"])
    )
    SessionsTbl = (
        db["SessionRow"].__table__
        if db.get("SessionRow")
        else Table("sessions", MetaData(), autoload_with=db["engine"])
    )

    # 1) ensure parent session exists (prevents FK errors)
    exists = sess.execute(
        select(SessionsTbl.c.sid).where(SessionsTbl.c.sid == sid)
    ).scalar_one_or_none()
    if not exists:
        sess.execute(
            SessionsTbl.insert().values(sid=sid, created_at=func.utc_timestamp())
        )

    # 2) upsert (sid, shot_idx)
    clean = {k: v for k, v in (fields or {}).items() if v is not None}
    stmt = (
        mysql_insert(ShotsTbl)
        .values(
            sid=sid, shot_idx=int(shot_idx), created_at=func.utc_timestamp(), **clean
        )
